...
pair = [(f1, pmi_exact), (f2, pmi_low), (f3, pmi_high)]
...
of1 = codecs.open('exact_unique.txt', 'w', 'utf8')
of2 = codecs.open('high_unique.txt', 'w', 'utf8')

for w in sorted(pmi_exact.iteritems(), key = lambda x: -x[1]):
    if w[0] not in pmi_high.keys():
        print >>of1, w[0], w[1]

for w in sorted(pmi_high.iteritems(), key = lambda x: -x[1]):
    if w[0] not in pmi_exact.keys():
        print >>of2, w[0], w[1]
